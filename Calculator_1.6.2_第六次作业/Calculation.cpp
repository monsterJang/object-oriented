/************************************************************
    ...
    Function List:
        ...
        6.void calculatingExpression(queue<string> q, bool is_ExceedMaxNumLen) : 主要的计算函数，计算表达式的值并输出
...
/*************************************************
    ...
    Ouput: 表达式的值
*************************************************/
void Calculation::calculatingExpression(queue<string> q, bool _isExceedMaxNumLen)
{
    if (_isExceedMaxNumLen)     // 若超过MaxNumLen位数的数字存在为真
    {
...
//    cout << endl;
    ...
    for (int i = expLen - 1; i >= 0; i--)
    {
        ...
        else if (isOperator(postfixExp[i]))     // 若该元素为运算符，则弹出数字栈的两个数进行相应的运算并将结果push进数字栈
        {
            ...
            {
            case '+':
                digitStack.push(1.00 * leftNum + rightNum);
                break;
            case '-':
                digitStack.push(1.00 * leftNum - rightNum);
                break;
        ...
        /*if (!digitStack.empty())
        {
            cout << "第" << i << "次：" << digitStack.top() <<endl;
        }*/
    }
